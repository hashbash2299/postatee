...
export default function PostateeApp() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setPosts(snap.docs.map(d => ({ id: d.id,...d.data(), user: users.hashem, time: "الآن", likes: 0, liked: false })));
    });
    return () => unsub();
  }, []);
  ...
  const handlePost = async (type = "text") => {
    ...
    await addDoc(collection(db, "posts"), { content, image, created_at: serverTimestamp() });
    setText("");
  };
      ...
      <div className="min-h-screen bg-[#050a0a] text-white" dir="rtl">
        <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/10 px-4 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <img src={users.hashem.avatar} className="w-8 h-8 rounded-full border border-cyan-400/30"/>
          </div>
        </header>
        ...
        <div className="max-w-[1300px] mx-auto flex gap-4 p-3">
          ...
          <div className="flex-1 max-w-[600px] mx-auto space-y-3">
            ...
            <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-3">
              <div className="flex gap-3"><img src={users.hashem.avatar} className="w-10 h-10 rounded-full"/><input value={text} onChange={e=>setText(e.target.value)} placeholder="بماذا تفكر يا هاشم؟" className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2.5 text-sm outline-none"/></div>
