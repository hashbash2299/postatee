          ...
          <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-3" onClick={e=>e.stopPropagation()}>
            ...
            {showFeelings && (
              <div className="mt-3 grid grid-cols-3 gap-2 bg-black/60 border border-white/10 p-3 rounded-2xl">
                {feelingsList.map(f=>(
                  <button key={f.id} onClick={()=>{setFeeling(f); setShowFeelings(false);}} className="flex items-center gap-2 p-2 rounded-xl hover:bg-white/10 text-sm"><span className="text-xl">{f.icon}</span> {f.label}</button>
                ))}
              </div>
            )}
          </div>
            ...
            <div key={post.id} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4" onClick={e=>e.stopPropagation()}>
              ...
              {editingPost?.id===post.id? (
                <div className="mt-3">
                  ...
                  <div className="flex gap-2 mt-2"><button onClick={async()=>{await updateDoc(doc(db,'posts',editingPost.id),{content:editingPost.content}); setEditingPost(null);}} className="bg-cyan-400 text-black px-4 py-1.5 rounded-full text-sm font-bold">حفظ</button><button onClick={()=>setEditingPost(null)} className="bg-white/10 px-4 py-1.5 rounded-full text-sm">إلغاء</button></div>
                </div>
