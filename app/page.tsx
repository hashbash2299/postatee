...
export default function Page(){
  ...
  return (
    <div className="min-h-screen bg-[#0B1418] text-white" dir="rtl">
      <TickerBar />
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@700;800&display=swap'); *{font-family:'Tajawal',sans-serif!important}`}</style>

      <header className="hidden lg:flex items-center justify-between px-6 py-3 bg-[#122025] border-b border-[#1A2E35] max-w-[1600px] mx-auto w-full">
        <div className="flex items-center gap-4">
          <Link href={`/profile/${currentUser?.uid}`} className="flex items-center gap-2"><img src={currentUser?.photoURL || `https://i.pravatar.cc/100?img=15`} className="w-10 h-10 rounded-full border-2 border-[#00E5FF]"/><span className="font-bold text-sm">{currentUser?.displayName}</span></Link>
      ...
      <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-[#122025] border-b border-[#1A2E35]">
