...
import { Search, Home, Video, Store, Users2, Heart, MessageCircle, Share2, MoreHorizontal, Crown, ShieldCheck, Star, CheckCircle2, ArrowRight, Plus } from "lucide-react";
...
export default function PostateeApp() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setPosts(snap.docs.map(d => ({ id: d.id,...d.data(), user: users.hashem, time: "الآن" })));
    });
    return () => unsub();
  }, []);
  ...
  const handlePost = async () => {
    if (!text.trim()) return;
    await addDoc(collection(db, "posts"), { content: text, created_at: serverTimestamp() });
    setText("");
  };
      ...
      <div className="min-h-screen bg-[#050a0a] text-white" dir="rtl">
        ...
        <div className="max-w-[1300px] mx-auto flex gap-4 p-3">
          <div className="hidden lg:block w-[260px] space-y-2 sticky top-[60px] h-fit">
            {Object.values(users).slice(0,4).map((u:any)=>(<div key={u.id} onClick={()=>openProfile(u)} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer"><img src={u.avatar} className="w-9 h-9 rounded-full"/><span className="text-sm font-bold">{u.name}</span><RoleBadge role={u.role}/></div>))}
          </div>
          ...
          <div className="flex-1 max-w-[600px] mx-auto space-y-3">
            ...
            <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-3 flex gap-3">
              <img src={users.hashem.avatar} className="w-10 h-10 rounded-full"/>
              <div className="flex-1 flex gap-2"><input value={text} onChange={e=>setText(e.target.value)} placeholder="بماذا تفكر يا هاشم؟" className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2.5 text-sm outline-none"/><button onClick={handlePost} className="bg-cyan-400 text-black font-black px-6 rounded-full">نشر</button></div>
            </div>
            ...
            {posts.map((post:any)=>(
              <div key={post.id} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4">
                ...
                <div className="flex gap-6 mt-4 pt-3 border-t border-white/5"><span className="flex gap-1.5 text-sm text-white/50"><Heart className="w-5 h-5"/>أعجبني</span><span className="flex gap-1.5 text-sm text-white/50"><MessageCircle className="w-5 h-5"/>تعليق</span><span className="flex gap-1.5 text-sm text-white/50"><Share2 className="w-5 h-5"/>مشاركة</span></div>
              </div>
            ))}
          </div>
          ...
          <div className="hidden xl:block w-[280px] sticky top-[60px] h-fit space-y-4">
            <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-3">
              ...
              <div className="space-y-1">{contacts.map((c:any)=>(<div key={c.id} onClick={()=>openProfile(c)} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer"><div className="relative"><img src={c.avatar} className="w-8 h-8 rounded-full"/><span className="absolute -bottom-0.5 -left-0.5 w-3 h-3 bg-green-500 rounded-full border-2 border-[#050a0a]"></span></div><span className="text-sm">{c.name}</span></div>))}</div>
            </div>
          </div>
        </div>
      </div>
