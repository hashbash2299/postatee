...
export default function LeftSidebar(){
  ...
  return (
    <div className="hidden lg:flex w-[280px] h-screen sticky top-0 flex-col bg-[#0B1418] border-l border-white/10 p-3">
      ...
      <nav className="flex-1 space-y-1 mt-2">
        ...
        <Link href={`/profile/${auth.currentUser?.uid}`} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-[14px] ${path?.startsWith('/profile')? 'bg-white text-black' : 'text-white/60 hover:bg-white/[0.06] hover:text-white'}`}>
          <User className="w-5 h-5"/> الملف الشخصي
        </Link>
      </nav>

      <button onClick={logout} className="flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 font-bold text-[14px]">
